...
public abstract class TSActiveRecordAb<T> implements TSActiveRecordIf<T>, Serializable {
	...
	public List<T> findAll(String... fieldsOrderBy) {
		...
		try {
			...
			queryObject = session.createQuery(fomatHQL(sqlFinal));
	...
	public List<T> findByModel(Map<String, Object> map, String... fieldsOrderBy) {
		...
		Criterion criterion = Example.create(this).enableLike(MatchMode.ANYWHERE).ignoreCase().excludeZeroes();
	...
	protected List<T> find(String query, String order, Object... objects) {
		...
		try {
			...
			session.setFlushMode(FlushMode.NEVER);
			queryObject = session.createQuery(fomatHQL(query));
	...
	protected T get(String hql, Object... objects) {
		...
		try {
			session.setFlushMode(FlushMode.NEVER);
			query = session.createQuery(fomatHQL(hql));
			if (objects != null) {
				int i = 1;
				for (Object o : objects) {
	...
	protected int deleteNamedQuery(String name, Object... objects) throws TSApplicationException {
		Query query = null;
		Session session = null;
		int qtdLines = 0;
		try {
	...
	protected int update(String hql, Object... objects) throws TSApplicationException {
		Query query = null;
		Session session = getSession();
		int qtdLines = 0;
		try {
	...
	protected int executeSQL(String sql, Object... objects) throws TSApplicationException {
		Query query = null;
		Session session = getSession();
		int qtdLines = 0;
		try {
	...
	protected int updateNamedQuery(String hql, Object... objects) throws TSApplicationException {
		Query query = null;
		Session session = getSession();
		int qtdLines = 0;
		try {
	...
	protected int delete(String hql, Object... objects) throws TSApplicationException {
		Query query = null;
		Session session = getSession();
		int qtdLines = 0;
		try {
	...
	private String fomatHQL(CharSequence queryString) {
		...
		StringBuilder strBuilder = new StringBuilder(queryString);

		int start = 0;
		int order = 1;

		while ((start = strBuilder.indexOf("?", start + 1)) > 0) {
			strBuilder.insert(start + 1, order);
			order++;
		}
